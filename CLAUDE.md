...
* Preserve official DepEd form layouts (SF1/SF2/SF4/SF8/SF9/SF10) — structural changes are a compliance risk, not a styling choice. Official forms must reproduce the authoritative reference (Excel/PDF export) as closely as browser rendering permits: font, font size, font weight, alignment, merged cells, table geometry, row heights, column widths, borders, wording, page size, margins, orientation, and print behavior are all controlled by the official source, not eyeballed or modernized. Application branding and Dark Mode must never alter an official form's paper appearance. Never redesign an official form for aesthetics or responsiveness. Measure against a real reference before claiming a fix — see `docs/OFFICIAL_FORM_FIDELITY.md` for what's actually been measured and verified per form, and its "Tooling and its limits" section before trusting `xlrd`'s bold-flag reads.
