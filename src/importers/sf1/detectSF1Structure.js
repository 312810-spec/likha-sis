...
export function detectSF1Structure(sheet) {
  ...
  const positional = hasCombinedNameColumn(rows[headerRow]);
  ...
  if (positional) {
    ...
    if (!Object.values(columnMap).includes("lastName")) {
  ...
  // --- Learner rows ---------------------------------------------------------
  // The header may occupy two rows (LIS), so start below whichever of the next
  // rows still looks like header labels rather than learner data.
  const dataStartRow = positional ? headerRow + 2 : headerRow + 1;
